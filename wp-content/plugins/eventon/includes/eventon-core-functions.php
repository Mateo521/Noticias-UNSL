	...
	function eventon_get_repeat_intervals($unix_S, $unix_E){
		...
		if($repeat_type=='custom'&& !empty($_POST['repeat_intervals'])){
			...
			for($x =0; $x<=$repeat_count; $x++){
				...
				if($repeat_type == 'mensual' && $month_repeat_by=='dow' && !empty($days) && is_array($days) ){
				...
				}elseif($repeat_type == 'semanal' && $week_repeat_by=='dow' && !empty($week_repeat_days) && is_array($week_repeat_days) ){
